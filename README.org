...
Centilisp, or just Centi, is a simple lisp dialect heavily inspired
by Kernel, Picolisp, Scheme and Common Lisp.

* Build

As of now, if you have SBCL installed, centi may be built with a script:
...
#+begin_src sh
  centi filename.centi
#+end_src

* Development

You can start centi with a repl by starting Common Lisp REPL and doing the following:

#+begin_src lisp
  ;; Add path to repository to asdf:*central-registry*
  (pushnew (pathname "/path/to/centi-repository.git")
...
Despite build script only working on SBCL, centi had originally been
developed both on SBCL and ECL using the approach above.
...
By conventions, symbols with capitalized names are type
identifiers.

Type identifier is any unique object that won't be possible to
reproduce - generated symbol, a particular pair or array, etc.
...
When you want to make it explicit that some top-level variable
is an implementation detail, you can use ':' prefix.

This is used for example in streams library to differenciate
between 'head' as the user-facing function and ':head' as raw
accessor to structure's head field.

For example:
- ~:print~
- ~:define~
- ~hashmap::new~ - imported form, typically won't happen
...
I've decided to use pair-based lists for Centi AST instead:
...
|             | Less ambiguity (e.g. ~(array:append "123" [4 5 6])~)      |                     |
...
Instead of incorporating OOP by default, Centi implements
generic functions, strongly resembling Clojure's 'defmulti'.

This approach has a simple implementation, while making it easy
to do any kind of dispatch - single dispatch, single dispatch
with inheritance, multiple dispatch, dispatch based on value,
etc. The cost is not letting the users extend how core dispatch
is done - for example, if your generic hardcodes dispatch based
on type of an object, you can't really add inheritance to it.

Unlike Clojure's implementation, dispatch function returns
the method itself instead of a signiture. This makes dispatch
more explicit and sometimes improves efficiency, like preventing
extra table lookup in case of 2 provided arguments to 'get'
generic.

** Generic functions

To simplify the interpreter logic, generics are implemented
in-language using a single global table mapping functions to
a pair of it's dispatch function and a table of signitures and
related methods:

#+begin_src scheme
  {#<function get> {(#<function> . [Pair head tail]) #<function alist:get>
                    (#<function> . Array)            #<function array:get>}}
#+end_src

In the future, it may be preferred to have generics be a special
record type instead.
...
fields: ptree, body, environment and ebind.
...
Environment is a stored parent environment, in which function
was created. It is used alongside ptree and ebind to construct
a new environment in which body will be evaluated.
...
`(wrap (special ...))`.
As an example, `(special () e (evaluate 'quote e))` is
equivalent to `quote` - it binds calling environment to variable
e, thus making is possible to evaluate other forms as if instead
of the call to special.
...
Record is like a general array, but it's first element is an
array where first element is pretty name of the record type
and rest are names of the fields.

Centi records do not support packing.
...
- replace every cl:error with cps
- make every parameter optional, defaulting to nil?
- look into algebraic effects
- IO system
- subtyping?
- hide private variables defined in bootstrap
- make all builtins come with a prefix to reduce overrides in
  library files
- make hashmap:set! resize the data array
...
- make all top level forms be evaluated at compile time, but main at
  runtime
...
- Make compiler do type inference to reduce type checks at runtime
